...
if __name__ == "__main__":
    ...
    rrt_folder = '/home/dimitria/PhD_codes/quadcopter_load/RRT_paths_2D_obstacle_1/*.yaml'
    rrt_df, rrt_summary = analyze_trajectories(rrt_folder, 'RRT Scenario 2')
    
    if rrt_summary is not None:
        print("\nFULL SUMMARY STATISTICS:")
        print(rrt_summary.to_string())
        
        print_paper_format(rrt_summary, "RRT Scenario 2")
        sanity_check_statistics(rrt_df, rrt_summary, "RRT")
        
        # Save results
        rrt_summary.to_csv('rrt_scenario_2_summary.csv')
        rrt_df.to_csv('rrt_scenario_2_all_metrics.csv', index=False)
        
        print(f"RRT results saved to:")
        print(f"  - rrt_scenario_2_summary.csv")
        print(f"  - rrt_scenario_2_all_metrics.csv\n")
    ...
    rrt_star_folder = '/home/dimitria/PhD_codes/quadcopter_load/RRT_star_paths_2D_obstacle_1/*.yaml'
    rrt_star_df, rrt_star_summary = analyze_trajectories(rrt_star_folder, 'RRT* Scenario 2')
    
    if rrt_star_summary is not None:
        print("\nFULL SUMMARY STATISTICS:")
        print(rrt_star_summary.to_string())
        
        print_paper_format(rrt_star_summary, "RRT* Scenario 2")
        sanity_check_statistics(rrt_star_df, rrt_star_summary, "RRT*")
        
        # Save results
        rrt_star_summary.to_csv('rrt_star_scenario_2_summary.csv')
        rrt_star_df.to_csv('rrt_star_scenario_2_all_metrics.csv', index=False)
        
        print(f"RRT* results saved to:")
        print(f"  - rrt_star_scenario_2_summary.csv")
        print(f"  - rrt_star_scenario_2_all_metrics.csv\n")
    ...
    est_df, est_summary = analyze_trajectories(est_folder, 'EST Scenario 2')
    
    if est_summary is not None:
        print("\nFULL SUMMARY STATISTICS:")
        print(est_summary.to_string())
        
        print_paper_format(est_summary, "EST Scenario 2")
        sanity_check_statistics(est_df, est_summary, "EST")
        
        # Save results
        est_summary.to_csv('est_scenario_2_summary.csv')
        est_df.to_csv('est_scenario_2_all_metrics.csv', index=False)
        
        print(f"EST results saved to:")
        print(f"  - est_scenario_2_summary.csv")
        print(f"  - est_scenario_2_all_metrics.csv\n")
    ...
    if len(results) >= 2:
        ...
        comparison_table.to_csv('algorithm_comparison_scenario_2.csv')
        print(f"\nComparison table saved to: algorithm_comparison_scenario_2.csv")
